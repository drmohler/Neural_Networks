...
if __name__ == '__main__':
    ...
    w8 = tf.Variable(tf.truncated_normal([hidden7,output])) 
    b8 = tf.Variable(tf.truncated_normal([1,output])) 
    ...
    #Cost vs Epoch Plot
    fig = plt.figure("Cost Vs Epoch for Immuno Network #3")
    plt.plot(cost_value)
    plt.xlabel('Epoch')
    plt.ylabel('Cost')
    plt.title("Cost Vs Epoch for Immuno Network #3")
    plt.grid()
    plt.show()
