...
if __name__ == '__main__':
    ...
    w2 = tf.Variable(tf.truncated_normal([hidden,hidden2])) #5x1
    b2 = tf.Variable(tf.truncated_normal([1,hidden2])) #scalar value

    w3 = tf.Variable(tf.truncated_normal([hidden2,hidden3])) #5x1
    b3 = tf.Variable(tf.truncated_normal([1,hidden3])) #scalar value

    w4 = tf.Variable(tf.truncated_normal([hidden3,output])) #5x1
    b4 = tf.Variable(tf.truncated_normal([1,output])) #scalar value
    ...
    #step = tf.train.AdagradOptimizer(0.5,0.1).minimize(cost)
    ...
    #Cost vs Epoch Plot
    fig = plt.figure("Cost Vs Epoch for Immuno Network #2")
    plt.plot(cost_value)
    plt.xlabel('Epoch')
    plt.ylabel('Cost')
    plt.title("Cost Vs Epoch for Immuno Network #2")
    plt.grid()
    plt.show()